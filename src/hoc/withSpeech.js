  ...
  const SpeechRecognitionInner = function (WrappedComponent) {
    ...
    return class SpeechRecognitionContainer extends Component {
      constructor(props) {
        ...
        var grammar = '#JSGF V1.0; grammar words; public <word> = ' + (WrappedComponent.props.words.map(word => word.name.toLowerCase()) || []).join('|') + ' ;'
      ...
      updateTranscript(event) {
        ...
        event.results[0].forEach(element => {
            WrappedComponent.propts.channel.publish(element.transcript)
        });
